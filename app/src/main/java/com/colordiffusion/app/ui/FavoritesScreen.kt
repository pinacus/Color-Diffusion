...
@Composable fun FavoritesScreen(viewModel: PaletteViewModel, onRestore: (PaletteEntity) -> Unit, onCopy: (PaletteEntity) -> Unit) {
    ...
    LazyColumn(Modifier.fillMaxSize().padding(horizontal = 20.dp), contentPadding = PaddingValues(vertical = 24.dp), verticalArrangement = Arrangement.spacedBy(18.dp)) {
        ...
        items(favorites, key = { it.id }) { palette -> FavoriteRow(palette, onRestore, onCopy, viewModel::deleteFavorite) }
    }
}

@Composable private fun FavoriteRow(palette: PaletteEntity, onRestore: (PaletteEntity) -> Unit, onCopy: (PaletteEntity) -> Unit, onDelete: (PaletteEntity) -> Unit) {
    Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
        ...
        IconButton(onClick = { onCopy(palette) }) { Icon(Icons.Default.ContentCopy, "Copy palette") }
        IconButton(onClick = { onRestore(palette) }) { Icon(Icons.Default.FileOpen, "Load palette") }
